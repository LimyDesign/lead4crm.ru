...
if (!$_COOKIE['_refid'] && $_SERVER['HTTP_REFERER']) {
...
if ($_SERVER['HTTP_REFERER']) {
    file_put_contents('/var/www/html/ref_debug.log', date('Y-m-d H:i:s') . ' => ' . $_SERVER['HTTP_REFERER'] . "\r\n", FILE_APPEND | LOCK_EX);
}
...
if ($cmd[0]) {
    if ($cmd[0] == 'ref') {
        ...
        unset($cmd[0], $cmd[1]);
        $new_url = implode('/', $cmd);
        header("Location: https://www.lead4crm.ru/".$new_url);
        exit;
    }
    ...
    if ($cmd[0] == 'logout') {
        ...
        exit;
    } elseif ($cmd[0] == 'withdrawals') {
        isAuth($cmd);
        header("Content-Type: application/json");
        echo json_encode($api->getWithdrawals($_SESSION['userid'], $cmd[1], $_REQUEST['sum']), JSON_UNESCAPED_UNICODE);
        exit;
        ...
        isAuth($cmd);
        header("Content-Type: application/json");
        echo json_encode($api->postURLReferal($_REQUEST['url'], $_SESSION['userid']), JSON_UNESCAPED_UNICODE);
        exit;
    } elseif ($cmd[0] == 'refUpdateURL') {
        isAuth($cmd);
        header("Content-Type: application/json");
        echo json_encode($api->postURLReferal($_REQUEST['url'], $_SESSION['userid'], true, $_REQUEST['id']), JSON_UNESCAPED_UNICODE);
        exit;
